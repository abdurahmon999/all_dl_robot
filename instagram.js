        ...
        if (get[0].type == 'Photo') {
          ...
          try {
            ...
            let buff = await getBuffer(get[0].url);
             fs.writeFileSync('content/vid-ig-single-' + chatId + '.mp4', buff)
            await bot.deleteMessage(chatId, load.message_id)
            await bot.sendChatAction(chatId, 'upload_video')
            await bot.sendVideo(chatId, 'content/vid-ig-single-' + chatId + '.mp4', { caption: 'Bu bot Threads, Twitter, Tiktok, Instagram, Facebook, Pinterest, Spotify dan video, audio, photo, musiqa yuklay oladi📥\nBot @all_dl_robot' })
             fs.unlinkSync('content/vid-ig-single-' + chatId + '.mp4')
          }
        }
      } else {
        get.forEach(maru => {
